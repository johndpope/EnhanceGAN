...
local Lab2RGB_module = torch.load('../dcgan.Lab2RGB.3layer/checkpoints/Lab2RGB_50_net_G.t7')
...
Lab2RGB_FULL:cuda()
local perceptual_loss_net = torch.load('../fast-neural-style/models/vgg16.t7')
...
for epoch = 1, opt.niter do
   ...
   while i <= math.min(data:size(), opt.ntrain) do
      ...
      print(string.format('[%d]-ErrG = %f, abs = %f, good, go and do D-update', generation_iter, errG[1], absERROR))
        while j < Diters and i <= math.min(data:size(), opt.ntrain) do
          ...
          print(string.format('[%d]-ErrG = %f, abs = %f, wait, go and redo G-update', generation_iter, errG[1],absERROR))
      end
      Giters = 1
      k = 0
