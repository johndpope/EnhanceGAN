...
AVA = torch.load('../ICCV-netD-2/checkpoints/model_best.t7')
AVA:add(nn.SoftMax())
AVA:evaluate()
AVA:cuda()
AVA_score_list1 = torch.Tensor(200, 2):zero()
AVA_score_list2 = torch.Tensor(200, 2):zero()
AVA_score_list3 = torch.Tensor(200, 1):zero()
AVA_score_list4 = torch.Tensor(200, 1):zero()


img_list = scandir('testing_img')
if file_exists('testing_img.t7') then
  ...
  all_test_image = torch.Tensor(200,3,224,224)
  test_image_sizes = torch.Tensor(200,3)
  for i = 1, 200 do
     print(string.format('Loading [%d/%d]\n', i, 200))
     I = image.load(string.format('testing_img/%s', img_list[i+2]))
...
netG_copy = torch.load(string.format('../fullconv6/checkpoints/FullConv6_10000_net_G.t7'))
for g = 1,50,49 do
  ds_timer:reset()
  ds_timer:resume()
  AVA_mean = {}
  ...
  netG_iter = string.format('FullConv9_%d_net_G', g*200)
  netG = torch.load(string.format('./checkpoints/%s.t7',netG_iter))
  netG:get(1):get(38).weight:copy(netG_copy:get(1):get(30).weight)
  -- print('Shithead, set num input dims 2 in nn.View(49)')
  -- netG:get(1):get(22):setNumInputDims(2)
  netG:evaluate()
  netG:cuda()
  os.execute("mkdir " .. string.format('result_%s', netG_iter))
  new_results = string.format('result_%s', netG_iter)
  for i = 1, 200 do

     original_size = test_image_sizes:narrow(1, i, 1)
     big_width = original_size[1][3]
     big_height = original_size[1][2]
     original_img = image.load(string.format('testing_img/%s', img_list[i+2]))
     ...
     for i=1,3 do
      ...
      I_cropped_lab[i]:div(meanstd.std[i])
                 ...
                 if g == 1 then
                     ...
                     AVA_score_list1:narrow(1,i,1):copy(AVA_score1)
     ...
     AVA_score_list2:narrow(1,i,1):copy(AVA_score2)
     -- Testing using VGG
     for i = 1,25 do
     ...
     AVA_score_list1[i][2], AVA_score_list2[i][2], 
  ...
  mean_score = AVA_score_list2:narrow(2,2,1):mean()
  std_score = AVA_score_list2:narrow(2,2,1):std()
  ...
  if g == 1 then
    original_mean = AVA_score_list1:narrow(2,2,1):mean()
    original_std = AVA_score_list1:narrow(2,2,1):std()
