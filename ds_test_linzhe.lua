...
AVA:evaluate()
AVA:cuda()
AVA_score_list1 = torch.Tensor(55, 1):zero()
AVA_score_list2 = torch.Tensor(55, 1):zero()

AVA_score_list3 = torch.Tensor(55, 1):zero()
AVA_score_list4 = torch.Tensor(55, 1):zero()



img_list = scandir('./cvpr-55-img')
...
-- else
  all_test_image = torch.Tensor(55,3,224,224)
  test_image_sizes = torch.Tensor(55,3)
  for i = 1, 55 do
     print(string.format('Loading [%d/%d]\n', i, 55))
     I = image.load(string.format('./cvpr-55-img/%s', img_list[i+2]), 3, 'float')
...
for g = 1,26, 25 do
  ...
  netG:evaluate()
  netG:cuda()
  os.execute("mkdir " .. string.format('LinZhe_result_%s', netG_iter))
  new_results = string.format('LinZhe_result_%s', netG_iter)
  for i = 1, 55 do

     original_size = test_image_sizes:narrow(1, i, 1)
     big_width = original_size[1][3]
     big_height = original_size[1][2]
     original_img = image.load(string.format('./cvpr-55-img/%s', img_list[i+2]), 3, 'float')
