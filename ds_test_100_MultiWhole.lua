...
total_img = 100;
testing_folder = 'testing_MultiWhole_100'
...
AVA:evaluate()
AVA:cuda()
AVA_score_list1 = torch.Tensor(total_img, 1):zero()
AVA_score_list2 = torch.Tensor(total_img, 1):zero()

AVA_score_list3 = torch.Tensor(total_img, 1):zero()
AVA_score_list4 = torch.Tensor(total_img, 1):zero()



img_list = scandir(string.format('%s', testing_folder))
if file_exists(string.format('%s.t7',testing_folder)) then
   print('Load saved .t7')
   all_test_image = torch.load(string.format('%s.t7',testing_folder))
   test_image_sizes = all_test_image.test_image_sizes
   all_test_image = all_test_image.all_test_image
else
  all_test_image = torch.Tensor(total_img,3,224,224)
  test_image_sizes = torch.Tensor(total_img,3)
  for i = 1, total_img do
     print(string.format('Loading [%d/%d]\n', i, total_img))
     I = image.load(string.format('%s/%s',testing_folder, img_list[i+2]))
  ...
  torch.save(string.format('%s.t7',testing_folder), {all_test_image=all_test_image, test_image_sizes=test_image_sizes})
end
...
for g = 1,26,25 do
  ...
  for i = 1, total_img do
     ...
     original_img = image.load(string.format('%s/%s', testing_folder, img_list[i+2]))
